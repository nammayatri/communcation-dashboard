...
const sendNotification = async (token, notificationJson, driverNotificationPayload, fcmAuthToken) => {
    try {
        const payload = {
            message: {
                token,
...
const processBatch = async (tokens, notificationJson, driverNotificationPayload, fcmAuthToken) => {
    const results = await Promise.all(tokens.map(async (token, index) => {
        ...
        while (retryCount < MAX_RETRIES) {
            try {
                ...
                const result = await sendNotification(token, notificationJson, driverNotificationPayload, fcmAuthToken);
                if (result.success) {
...
export const processCSVAndSendNotifications = async (file, config, fcmAuthToken, onProgress) => {
            ...
            complete: async (parseResults) => {
                try {
                    ...
                    for (let i = 0; i < tokens.length; i += BATCH_SIZE) {
                        ...
                        const batchResults = await processBatch(batch, notificationJson, driverNotificationPayload, fcmAuthToken);
