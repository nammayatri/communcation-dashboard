...
const saveCSVToIndexedDB = async (id, csvContent) => {
    if (!db)
        await initDB();
    return new Promise((resolve, reject) => {
...
const getCSVFromIndexedDB = async (id) => {
    if (!db)
        await initDB();
    return new Promise((resolve, reject) => {
...
const deleteCSVFromIndexedDB = async (id) => {
    if (!db)
        await initDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(CSV_STORE, 'readwrite');
...
const cleanupOldData = () => {
    try {
        ...
        if (schedules.length > MAX_STORED_OVERLAYS) {
            ...
            const sortedSchedules = schedules.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
const processBatch = async (csvContent, overlayConfig, fcmToken, batchStart, batchEnd) => {
    ...
    try {
        const result = await processCSVAndSendNotifications(batchFile, overlayConfig, fcmToken);
        return result;
    }
    catch (error) {
...
const processScheduledOverlay = async (schedule) => {
    try {
        ...
        for (let i = 0; i < batches.length; i += MAX_PARALLEL_BATCHES) {
            ...
            const batchPromises = currentBatches.map(([start, end]) => processBatch(csvContent, schedule.overlayConfig, schedule.fcmToken, start, end));
            ...
            const schedules = getScheduledOverlays();
            const updatedSchedules = schedules.map(s => s.id === schedule.id
                ? {
                    ...s,
                    status: 'processing',
                    result: {
    ...
    schedulerInterval = setInterval(() => {
        ...
        schedules.forEach(schedule => {
            if (schedule.status === 'pending' && new Date(schedule.scheduledTime) <= now) {
                ...
                const updatedSchedules = schedules.map(s => s.id === schedule.id ? { ...s, status: 'processing' } : s);
                localStorage.setItem(STORAGE_KEY, JSON.stringify(updatedSchedules));
                // Process the overlay
                processScheduledOverlay(schedule);
            }
...
export const scheduleOverlay = async (overlayConfig, csvFile, fcmToken, scheduledTime) => {
    try {
        // Validate inputs
...
export const getScheduledOverlays = () => {
    try {
        const storedSchedules = localStorage.getItem(STORAGE_KEY);
...
export const cancelScheduledOverlay = async (id) => {
    try {
        const schedules = getScheduledOverlays();
